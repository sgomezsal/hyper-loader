...
struct Args {
    ...
    /// pulse rings and the digit counter.
    #[arg(long, default_value = "7ed4c8")]
    accent: String,
    ...
    /// Fade-in duration in milliseconds for the overlay decorations.
    ...
    /// Alpha (0.0-1.0) of the dark overlay drawn over the blurred screenshot background.
...
impl CompositorHandler for AppState {
    ...
    fn frame(&mut self, conn: &Connection, qh: &QueueHandle<Self>, surface: &wl_surface::WlSurface, _: u32) {
        self.frame += 1;
        self.counter_seed = self.counter_seed.wrapping_mul(6364136223846793005).wrapping_add(1442695040888963407);
...
impl LayerShellHandler for AppState {
    ...
    fn configure(&mut self, conn: &Connection, qh: &QueueHandle<Self>, layer: &LayerSurface, configure: LayerSurfaceConfigure, _: u32) {
        self.width = configure.new_size.0.max(1);
        self.height = configure.new_size.1.max(1);
        self.configured = true;
        let _ = std::fs::write(READY_PATH, "1");
        self.render(conn, qh, layer.wl_surface());
    }
}
...
impl AppState {
    fn render(&mut self, _conn: &Connection, qh: &QueueHandle<Self>, surface: &wl_surface::WlSurface) {
        ...
        let fade_alpha = ((t * 1000.0) / self.fade_ms).min(1.0);
        let bg_fade = self.bg_fade_start.map(|s| {
            (s.elapsed().as_secs_f32() / 0.6).min(1.0)
        }).unwrap_or(0.0);

        let accent = self.accent;
        let accent2 = self.accent2;
        let glitch = self.glitch;
        let overlay_alpha = self.overlay_alpha;
        let fh = self.height;
        let seed = self.counter_seed;
        let no_counter = self.no_counter;
        ...
        draw_frame(&mut pixmap, t, fade_alpha, bg_fade, accent, accent2, glitch, overlay_alpha, fh, bg, gif_frame, seed, no_counter);
        ...
        for x in 0..w as i32 {
            ...
            for dy in -r..=r {
                for dx in -r..=r {
                    ...
                    rs += p[0] as u64; gs += p[1] as u64; bs += p[2] as u64;
                }
            }
            let p = out.get_pixel_mut(x as u32, y as u32);
            p[0] = (rs / area) as u8; p[1] = (gs / area) as u8;
            p[2] = (bs / area) as u8; p[3] = 255;
        }
    }
    out
}
...
fn draw_frame(
        ...
        if bg_fade > 0.0 {
            ...
            for py in 0..ph {
                for px in 0..pw {
                    ...
                    let blend = bg_fade;
                    let nr = p[2] as f32 / 255.0 * blend + existing.red() as f32 / 255.0 * (1.0 - blend);
                    let ng = p[1] as f32 / 255.0 * blend + existing.green() as f32 / 255.0 * (1.0 - blend);
                    let nb = p[0] as f32 / 255.0 * blend + existing.blue() as f32 / 255.0 * (1.0 - blend);
                    pixels[idx] = ColorU8::from_rgba(
                        (nb * 255.0) as u8, (ng * 255.0) as u8, (nr * 255.0) as u8, 255
                    ).premultiply();
                }
            }
            ...
            pixmap.fill_rect(Rect::from_xywh(0.0, 0.0, w, h).unwrap(), &overlay, Transform::identity(), None);
            ...
            if glitch {
                ...
                pixmap.fill_rect(Rect::from_xywh(0.0, 0.0, w, h).unwrap(), &tint, Transform::identity(), None);
            }
        }
    }
    ...
    while x < w {
        ...
        pb.move_to(x, 0.0); pb.line_to(x, h);
    ...
    while y < h {
        let mut pb = PathBuilder::new();
        pb.move_to(0.0, y); pb.line_to(w, y);
        if let Some(path) = pb.finish() {
            pixmap.stroke_path(&path, &grid_paint, &stroke, Transform::identity(), None);
        }
        y += grid_size;
    ...
    let cs = 60.0f32; let m = 30.0f32; let cw = 2.0f32;
    pixmap.fill_rect(Rect::from_xywh(m, m, cs, cw).unwrap(), &cp, Transform::identity(), None);
    pixmap.fill_rect(Rect::from_xywh(m, m, cw, cs).unwrap(), &cp, Transform::identity(), None);
    pixmap.fill_rect(Rect::from_xywh(w-m-cs, m, cs, cw).unwrap(), &cp, Transform::identity(), None);
    pixmap.fill_rect(Rect::from_xywh(w-m-cw, m, cw, cs).unwrap(), &cp, Transform::identity(), None);
    pixmap.fill_rect(Rect::from_xywh(m, h-m-cw, cs, cw).unwrap(), &cp, Transform::identity(), None);
    pixmap.fill_rect(Rect::from_xywh(m, h-m-cs, cw, cs).unwrap(), &cp, Transform::identity(), None);
    pixmap.fill_rect(Rect::from_xywh(w-m-cs, h-m-cw, cs, cw).unwrap(), &cp, Transform::identity(), None);
    pixmap.fill_rect(Rect::from_xywh(w-m-cw, h-m-cs, cw, cs).unwrap(), &cp, Transform::identity(), None);
    ...
    for i in 0..3 {
        ...
        rp.anti_alias = true;
        let mut rs = Stroke::default();
        rs.width = 1.5;
        let mut pb = PathBuilder::new();
        pb.push_oval(Rect::from_xywh(w/2.0-radius, h/2.0-radius, radius*2.0, radius*2.0).unwrap());
        if let Some(path) = pb.finish() {
    ...
    if let Some(gif) = gif_frame {
        ...
        for dy in 0..dh as i32 {
            for dx in 0..dw as i32 {
                ...
                let existing = pixels[idx].demultiply();
                let nr = (p[0] as f32 / 255.0 * ga + existing.red() as f32 / 255.0 * (1.0 - ga)).clamp(0.0, 1.0);
                let ng = (p[1] as f32 / 255.0 * ga + existing.green() as f32 / 255.0 * (1.0 - ga)).clamp(0.0, 1.0);
                let nb = (p[2] as f32 / 255.0 * ga + existing.blue() as f32 / 255.0 * (1.0 - ga)).clamp(0.0, 1.0);
                pixels[idx] = ColorU8::from_rgba(
                    (nb * 255.0) as u8, (ng * 255.0) as u8, (nr * 255.0) as u8, 255
                ).premultiply();
            }
    ...
    if !no_counter {
        ...
        for col in 0..num_cols {
            ...
            for row in 0..digit_count {
                ...
                let alpha_row = if row == digit_count - 1 { a(0.9) }
                               else if row == digit_count - 2 { a(0.5) }
                               else { a(0.12) };
    ...
    if glitch {
        ...
        pixmap.fill_rect(Rect::from_xywh(0.0, scan_y, w, 2.0).unwrap(), &sp, Transform::identity(), None);
        for i in 0..3u32 {
            ...
            pixmap.fill_rect(Rect::from_xywh(gx, gy, gl, 1.0).unwrap(), &gp, Transform::identity(), None);
        }
    }
}
...
fn draw_pixel_digit(pixmap: &mut PixmapMut, digit: u8, x: f32, y: f32, r: f32, g: f32, b: f32, alpha: f32) {
...
fn main() {
    ...
    let gif_frames: Vec<(RgbaImage, u32)> = args.gif.as_ref().and_then(|p| {
        File::open(p).ok().map(|f| {
            let reader = BufReader::new(f);
            GifDecoder::new(reader).ok()
    ...
    let layer_surface = layer_shell.create_layer_surface(
        &qh, surface, Layer::Overlay, Some("hyper-loader"), None,
    );
    ...
    let pool = SlotPool::new(1920 * 1080 * 4 * 2, &shm).unwrap();

    let accent = parse_hex(&args.accent);
    let accent2 = args.accent2.as_deref().map(parse_hex).unwrap_or(accent);
    ...
    loop {
        event_loop.dispatch(Some(Duration::from_millis(16)), &mut app).unwrap();
        if app.exit { break; }
    }
